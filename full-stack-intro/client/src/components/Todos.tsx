...
export function Todos() {
  ...
  useEffect(() => {
    async function fetchApi() {
      try {
        ...
        if (!response.ok) {
          throw new Error('api has weird problem');
        }
  ...
  async function addTodo(newTodo: UnsavedTodo) {
    try {
      ...
      if (!response.ok) {
        throw new Error('response problem in addTodo');
      }
  ...
  async function toggleCompleted(todo: Todo) {
    try {
      ...
      if (!response.ok) {
        throw new Error('response problem in toggleCompleted');
      }
      const data = await response.json();

      setTodos((prevTodos) =>
        prevTodos.map((t) => (t.todoId === data.todoId ? data : t))
      );
    } catch (error) {
      setError(error);
    }
